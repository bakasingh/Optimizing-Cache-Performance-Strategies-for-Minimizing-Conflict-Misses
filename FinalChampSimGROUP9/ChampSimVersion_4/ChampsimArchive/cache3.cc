...
//apna boht naya code
    int m = LLC_SET; // Number of rows in the matrix
    int p = LLC_SET; // Upper bound for random numbers
    vector<vector<uint32_t>> matrix(m, vector<uint32_t>(2));

    random_device rd;
    mt19937 gen(rd());
    uniform_int_distribution<uint32_t> dist(0, p-1);
...
void CACHE::handle_fill()
{
  ...
  if(cache_type == IS_LLC){
        ...
        // cout<<"Earlier set: "<<set<<endl;
        // cout<<"Searcher flag status: "<<searcher_flag<<endl;
...
void CACHE::handle_writeback()
{
    ...
    if ((WQ.entry[WQ.head].event_cycle <= current_core_cycle[writeback_cpu]) && (WQ.occupancy > 0)) {
        ...
        // access cache
            ...
            // OOPs not here it is in read instead
       //here we need some function which checks whether given element is in given set or not 
       //if it is not in given set then increment set no by 1
    ...
    if(cache_type == IS_LLC){
        ...
        // if(check_way==NUM_WAY){ // element not found in set
