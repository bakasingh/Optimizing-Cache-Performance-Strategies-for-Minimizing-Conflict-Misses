...
int flag=1;
//new code
long long count_fill=0;
int my_index=0;
int prev_index=0;

void CACHE::renew_set(uint32_t set,uint64_t tag){
    uint32_t way = 0;
    
    // fill invalid line first
    linked[set]=0;
    for (way=0; way<NUM_WAY; way++) {
        if (block[set][way].valid !=false&&block[set][way].address==tag) {
...
uint32_t CACHE::find_cold(){
  
  map<uint32_t,int>::iterator it = evictionCount.begin();
      while (it != evictionCount.end()) {
         
        //  try{
         if(linked[it->first]<2){ //if the found set is not mapped to anyone
...
//apna naya code
//This function is specifically check hits for modified sets and returns the matched way
//It is almost similar to Check_hit function
int CACHE::check_my_hit(uint32_t set,PACKET *packet)
{
...
//apna code
//This function searched whether there is vacant space in the set or not
//if there is no vacant space then it returns NUMWAY which is interepreted properly where it is called
int CACHE::searcher( uint32_t set )
{   
...
void CACHE::handle_fill()
{
    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
       ...
       //Make a searcher function which search whether there is space or not in the given set
       //if there isn't any space then give the  next set

        uint32_t set = get_set(MSHR.entry[mshr_index].address), way;
        uint32_t org_set = get_set(MSHR.entry[mshr_index].address);

  //changes
  //Here we are doing the task of mapping the given set to its consecutive sets depending upon the availability of  space for filling
  if(cache_type == IS_LLC){

         
         
         int searcher_flag=searcher(set);
         
         if(searcher_flag==0){//no space in the set so map it 
              evictionCount[set]++;
              //  cout<<"evicted set: "<<set<<endl;
              if(Remap_Table[set]==3000){//it is not mapped yet
                    
                    uint32_t mapped_set=find_cold();
                    if(mapped_set!=4000){
                    Remap_Table[set]=mapped_set;
                    set=Remap_Table[set];
                    }
                    // else{
                    //   Remap_Table[set]=set;
                    //   // cout<<"yes no space here"<<endl;
                    // }
                ...
                if((distance(evictionCount.begin(),evictionCount.find(Remap_Table[set])))>100){
                  uint32_t oldset=Remap_Table[set];
              
                  Remap_Table[set]=find_cold();
                  // cout<<"From: "<<oldset<<"to: "<<Remap_Table[set]<<endl;
                  // cout<<t<<endl;
                  renew_set(oldset,MSHR.entry[mshr_index].address);
                  
                 
                }
              }
              
             if(searcher(Remap_Table[set]==0)){ 
                ...
                flag=1-flag;
                    ...
                    if(Remap_Table2[org_set]==3000){//it is not mapped yet
                    
                    uint32_t mapped_set=find_cold();
                    if(mapped_set!=4000){
                    Remap_Table[org_set]=mapped_set;
                    set=Remap_Table[org_set];
                    }
                ...
                if((distance(evictionCount.begin(),evictionCount.find(Remap_Table2[org_set])))>50){
                  uint32_t oldset=Remap_Table[org_set];
              
                  Remap_Table[org_set]=find_cold();
                  // cout<<"From: "<<oldset<<"to: "<<Remap_Table[set]<<endl;
                  // cout<<t<<endl;
                  renew_set(oldset,MSHR.entry[mshr_index].address);
        ...
        // if(searcher_flag==0&&set<=NUM_SET-2){

        // set=set+1;
         
        //           searcher_flag=searcher(set);
        //           if(searcher_flag==0&&set<=NUM_SET-2){
...
void CACHE::handle_writeback()
{
    ...
    if ((WQ.entry[WQ.head].event_cycle <= current_core_cycle[writeback_cpu]) && (WQ.occupancy > 0)) {
        ...
        uint32_t org_set = get_set(WQ.entry[index].address);

//change
//Here using checkway parameter we are checking whether the given element is present in the given set or not 
//if it  is not then the set is changed to its mapped set

  if(cache_type == IS_LLC){

      // uint32_t set = get_set(WQ.entry[index].address);
       uint32_t check_way=get_way(WQ.entry[index].address,set);
      //  int searcher_flag=searcher(set);
        if(check_way==NUM_WAY){
          MissCount[set]++;
        }

        
        if(check_way==NUM_WAY&&Remap_Table[set]!=3000){ // element not found in set
           
           set=Remap_Table[set];
           check_way=get_way(WQ.entry[index].address,set);

           if(check_way==NUM_WAY&&Remap_Table2[org_set]!=3000){ // element not found in set
           
           set=Remap_Table2[org_set];
        ...
        // if(check_way==NUM_WAY&&set<=NUM_SET-2){ // element not fou in set
        // set=set+1;
        // check_way=get_way(WQ.entry[index].address,set);
        // if(check_way==NUM_WAY&&set<=NUM_SET-2){ // element not found in set
        // set=set+1; 
        ...
        //my changes
        if(cache_type==IS_LLC){
          way=check_my_hit(set,&WQ.entry[index]);
        }
        //my changes
    ...
    //     int searcher_flag=searcher(set);
    //     // cout<<"Earlier set New: "<<set<<endl;
    //     if(searcher_flag==0&&set<=NUM_SET-2-1){
    //     set=set+1;
    ...
    for (uint32_t i=0; i<MAX_READ; i++) {
      ...
      if (read_cpu == NUM_CPUS)
        ...
        if ((RQ.entry[RQ.head].event_cycle <= current_core_cycle[read_cpu]) && (RQ.occupancy > 0)) {
            ...
            uint32_t org_set = get_set(RQ.entry[index].address);

//change

//Here same thing is done like we did in Writeback function
//  uint32_t check_way=get_way(RQ.entry[index].address,set);
       ...
       uint32_t check_way=get_way(RQ.entry[index].address,set);
        ...
        // }
         if(check_way==NUM_WAY&&Remap_Table[set]!=3000){ // element not found in set
           ...
           set=Remap_Table[set];
           check_way=get_way(RQ.entry[index].address,set);
        ...
        // cout<<"Error in handle read"<<endl;
 
        // cout<<"Set: "<<Remap_Table[set]<<endl;
...
//change
        ...
        //my changes
        if(cache_type==IS_LLC){
          way=check_my_hit(set,&RQ.entry[index]);
        }
        //my changes
...
uint32_t CACHE::get_set(uint64_t address)
{
    ...
    if(cache_type==IS_LLC){
    accessCount[set]++; //here we are increasing the global access count for the corresponding set Only for LLC CACHE
    
    }
    // cout<<"Returned set: "<<set<<endl;
    return set;
