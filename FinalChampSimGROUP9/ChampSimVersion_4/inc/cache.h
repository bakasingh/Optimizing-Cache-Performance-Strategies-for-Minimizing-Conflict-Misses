...
class CACHE : public MEMORY {
  public:
    ...
    int rows=LLC_SET;
//apna code
//Important parameters needed to keep track of set accesses and eviction counts
//    vector<vector<uint32_t>> Remap_Table(rows, vector<int>(cols));
vector<uint32_t> Remap_Table;
vector<uint32_t> Remap_Table2;
// vector<vector<uint32_t>> Remap_Table = vector<vector<uint32_t>>(rows,vector<uint32_t>(cols,3000));
   map<uint32_t, int> accessCount; // Set -> access count
   map<uint32_t, int> evictionCount; // Set -> eviction count
   map<uint32_t, int> MissCount; // Set -> eviction count
   vector<uint32_t> linked;
//apna code
    ...
    //    Remap_Table = vector<vector<uint32_t>>(rows,vector<uint32_t>(cols,3000));
       Remap_Table = vector<uint32_t> (LLC_SET,3000);
       Remap_Table2 = vector<uint32_t> (LLC_SET,3000);
        //my code 
        //  Remap_Table = vector<vector<uint32_t>>(LLC_SET,);
        //  Remap_Table = vector<vector<uint32_t>>(LLC_SET, vector<uint32_t>(cols,3000));
         linked=vector<uint32_t>(LLC_SET,0);
...
//         Remap_Table(vector<vector<uint32_t>>(LLC_SET, vector<uint32_t>(cols, -1)))
//           for (int i = 0; i < LLC_SET; ++i) {
...
//   }
    ...
    //my changes
    int check_my_hit(uint32_t set,PACKET *packet);
    uint32_t find_cold();
    //my changes
    ...
    int searcher( uint32_t set );//changes  Searcher is our own function
    void  renew_set( uint32_t set,uint64_t tag );//changes  Searcher is our own function
    void printer(); //printer is also our own function which prints the accesses to set in access count file
    void printer2(); //printer is also our own function which prints the accesses to set in access count file
    void printer3(); //printer is also our own function which prints the accesses to set in access count file
    void printer4(); //printer is also our own function which prints the accesses to set in access count file
    void printer6(); //printer is also our own function which prints the accesses to set in access count file
};
