...
uint32_t CACHE::lru_victim(uint32_t cpu, uint64_t instr_id, uint32_t set, const BLOCK *current_set, uint64_t ip, uint64_t full_addr, uint32_t type)
{
    ...
    // cout<<"sET recieved LOC 0: "<<set<<endl;
    // }
    ...
    // cout<<"sET recieved LOC 1: "<<set<<endl;
    // }
    
    if (way == NUM_WAY) {
    // cout<<"sET recieved: "<<set<<endl;
        // int alter=rand()%2;
        // if(alter==0){
        for (way=0; way<NUM_WAY; way++) {
            if (block[set][way].lru == NUM_WAY-1) {
                ...
                // // cout<<"sET recieved LOC 2: "<<set<<endl;
                // //    cout<<"set: "<<set<<endl;
                //    evictionCount[set]++; //here we are keeping track of eviction done in the given set Though we are not printing it For Now
                // }
                DP ( if (warmup_complete[cpu]) {
            ...
            // cout<<"Double check OUter loop"<<endl;
